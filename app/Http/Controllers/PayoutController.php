...
class PayoutController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:payout view', only: ['index', 'show']),
            new Middleware('permission:payout delete', only: ['destroy']),
        ];
    }
    public function index(Request $request)
    {
        ...
        if ($search) {
            $query->where(function ($sub_query) use ($search) {
                return $sub_query->where('order_id', 'LIKE', "%{$search}%")->orWhere('id', 'LIKE', "%{$search}%")->orWhere('tran_id', $search)->orWhere('shop_id', 'LIKE', "%{$search}%");
            });
        }
    ...
    public function shop_payouts(Request $request)
    {
        $search = $request->input('search', '');
        $sortBy = $request->input('sortBy', 'id');
        $sortDirection = $request->input('sortDirection', 'desc');
        ...
        return Inertia::render('admin/shop_payouts/Index', [
            'tableData' => $tableData,
        ]);
    }
